...
class AslDb(object):
    ...
    def __init__(self,
        ...
        """
        # This dataframe contains the complete database. However, the data is
        # UNLABELLED.
...
class WordsData(object):
    ...
    def __init__(self, asl:AslDb, csvfile:str, feature_list:list):
        ...
        """
        # self._data contains a list of sequences for each word. Each of those
        # sequences contains a list of frames, which in turn contains a list of
        # features.
        self._data = self._load_data(asl, csvfile, feature_list)
        # self._hmm_data contains the same information as self._data, but in a
        # format suitable for the hmmlearn lib: word->(X, lengths).
        self._hmm_data = create_hmmlearn_data(self._data)
        # self.num_items is simply the number of words included in the data.
        self.num_items = len(self._data)
        # self.words is a list of words included in the data.
    ...
    def _load_data(self, asl, fn, feature_list):
        ...
        :return: dict of word->sequences->frames->features
        """
...
class SinglesData(object):
    ...
    The difference to the WordsData is, that with SinglesData we index by indices
    instead of words. Also as far as I understand similar words are not necessarily
    grouped under the same index.
    """
    ...
    def __init__(self, asl:AslDb, csvfile:str, feature_list):
        ...
        # self.wordslist is a list containing all words (with duplicates) included
        # in the dataset.
        self.wordlist = list(self.df['word'])
        # self.sentences_index is a dictionary mapping video indices to a list of
        # sequence indices as they can be used to index self._data.
        # {v0: [i0, i1, i2], v1: [i0, i1, i2], ... ,}
        self.sentences_index  = self._load_sentence_word_indices()
        # self._data is a dictionary mapping indices to a list of sequences
        self._data = self._load_data(asl, feature_list)
        # self._hmm_data contains the same information as self._data but in a format
        # suitable for hmmlearn lib: dict of idx->(X, lengths)
        self._hmm_data = create_hmmlearn_data(self._data)
        # self.num_items contains the number of entries in the csv file. This is the
        # same as the number of words (including duplicates).
        self.num_items = len(self._data)
        # self.num_sentences contains the number of videos, which is the same as the
        # number of sentences.
    ...
    def _load_sentence_word_indices(self):
        """
        Returns a dictionary mapping video indices to a list of sequence indices.
...
def combine_sequences(sequences):
    '''
    Takes a list of sequences and flattens it. Then it returns a tuple of the
    flattened list and a list containing the length of all included sequences.
...
def create_hmmlearn_data(dict):
    '''
    For each word in dict, takes all sequences and returns them in the format
    X, lengths.
    '''
    seq_len_dict = {}
    for key in dict:
