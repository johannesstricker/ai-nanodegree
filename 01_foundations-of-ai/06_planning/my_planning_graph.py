...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Test if this a_node can result from the current s_level.
            if a_node.prenodes.issubset(self.s_levels[-1]):
                # Add a_node to a_level.
                self.a_levels[-1].add(a_node)
                # Add a_node as child to all nodes of s_level and add those nodes
                # to a_node's parents.
    ...
    def add_literal_level(self, level):
        ...
        """
        # Use a list for the new s_level, to be able to mutate the s_node's parents.
        s_level = []
        for a_node in self.a_levels[-1]:
            for s_node in a_node.effnodes:
                # Only append the node if it's not already in there.
                if s_node not in s_level:
                    s_level.append(s_node)
                # Take the s_node and append a_node to it's parents.
                idx = s_level.index(s_node)
                s_level[idx].parents.add(a_node)
                # Append s_node to a_node's children.
                a_node.children.add(s_level[idx])
        # Make a set from s_level and append it.
        self.s_levels.append(set(s_level))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Count the elements in node_a1's added effects, that are also in node_a2's removed effects
        lhs = len([effect for effect in node_a1.action.effect_add if effect in node_a2.action.effect_rem])
        # Count the elements in node_a2's added effects, that are also in node_a1's removed effects
        rhs = len([effect for effect in node_a2.action.effect_add if effect in node_a1.action.effect_rem])
        # Return true if any of these effects exsist.
        return lhs + rhs > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Count the elements in node_a1's positive preconditions, that are removed by node_a2.
        A = len([effect for effect in node_a1.action.precond_pos if effect in node_a2.action.effect_rem])
        # Count the elements in node_a1's negative preconditions, that are added by node_a2.
        B = len([effect for effect in node_a1.action.precond_neg if effect in node_a2.action.effect_add])
        # Count the elements in node_a2's positive preconditions, that are removed by node_a1.
        C = len([effect for effect in node_a2.action.precond_pos if effect in node_a1.action.effect_rem])
        # Count the elements in node_a2's negative preconditions, that are added by node_a1.
        D = len([effect for effect in node_a2.action.precond_neg if effect in node_a1.action.effect_add])
        # Return true if any of these elements exist.
        return A + B + C + D > 0
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Iterate through all of a1's parents p1 and see if a2 has any parents which are
        # mutex to p1.
        for p1 in node_a1.parents:
            mutex_parents = [p2 for p2 in node_a2.parents if p1.is_mutex(p2)]
            # If any of those mutex parents exist, return True.
            if len(mutex_parents) > 0:
                return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Check if both nodes have the same symbol and one is positive and the other negative.
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # For each of s1's parent nodes a1, count all of s2's parents, that are either equal or not mutex to a1.
        for node_a1 in node_s1.parents:
            possible_actions = [node_a2 for node_a2 in node_s2.parents if not node_a1.is_mutex(node_a2) or node_a1 == node_a2]
            # If any of those actions exist, the state literals are not mutally exlusive.
            if len(possible_actions) > 0:
                return False
        # For each of s2's parent nodes a2, count all of s1's parents, that are either equal or not mutex to a2.
        for node_a2 in node_s2.parents:
            possible_actions = [node_a1 for node_a1 in node_s1.parents if not node_a2.is_mutex(node_a1) or node_a2 == node_a1]
            # If any of those actions exist, the state literals are not mutally exlusive.
            if len(possible_actions) > 0:
                return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        # For each of these goals, find the s_level at which it has been satisfied.
        for goal in goals:
            goal_satisfied = False
            # Search all s_levels.
            for idx, level in enumerate(self.s_levels):
                # Create a list of expressions that need to be satisfied for this level.
                # For all positive s_nodes add their symbol and for all negative
                # s_nodes add their negative symbol.
                level_expressions = [s_node.symbol for s_node in level if s_node.is_pos == True] + [(not s_node.symbol) for s_node in level if s_node.is_pos == False]
                # If goal is equal to any of these expressions, it is satisfied
                # at this level.
                if goal in level_expressions:
                    goal_satisfied = True
                    level_sum += idx
                    break
            # If this goal could not be satisfied, return False.
